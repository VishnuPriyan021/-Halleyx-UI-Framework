<template>
  <div
    ...
    :class="{ 'collapse-shrink': this.collapseState ,
    ...
    <nav class="nav">
      <ul class="nav-items">
        <li
          ...
          :class="item.separator == true?'separator':item.separator==undefined?'':''"
        >
          <a href="#" class="nav_link" :class="{active:item.active}" ref="nav-link">
...
export default {
  ...
  methods: {
    ...
    navTo(item, $event) {
      ...
      console.log(this.$refs['nav-link']);
      this.$refs['nav-link']
      ...
      ele =
        ele.tagName == "I"
          ...
          : ele.tagName == "SPAN"
          ? ele.parentElement.classList.add("active")
          : ele.tagName == "LI" ? ele.classList.children[0].add("active"):ele.classList.add("active");
    },
    goToHome() {
      this.$emit("goToHome");
    },
