...
<div class="hlx-select-container" @click="OptionContainer" v-if="this.single_select === true && this.clearable === true" @mouseover="clearIcon" @mouseout="arrowIcon" >
  ...
  <div class="hlx-select-input-placeholder" v-if="this.placeholder === true">{{this.placeholder_value}}</div>
  <div class="hlx-select-input"  v-show="this.dropdownvalue === true"><i :class="this.icon_value"></i>{{this.option_value}}</div>
  ...
  <div class="hlx-select-icon" v-if="this.clearable === false">
  <i class="icon-angle-down-regular" :id="'arrowid'+this.unique"></i>
...
<div class="hlx-select-container" @click="OptionContainer" v-if="this.icon === true">
  <p class="title-name" :id="'title1'+this.unique" v-show="this.dropdownname === true">{{this.title}}</p>
  <div class="hlx-select-input-placeholder" v-if="this.placeholder === true">{{this.placeholder_value}}</div>
  <div class="hlx-select-input" v-if="this.dropdownvalue === true"><CountryFlag :iso="this.icon_country_flag" /></div>
  <div class="hlx-select-icon">
...
<!-----------------------------------------------------------------Multiple select--------------------------------------------------------------------->
<div class="hlx-multi-select-container" @click="MultiselectOptioncontainer" v-if="this.multi_select === true" :id="'multiselect'+this.unique">
  <p class="title-name" v-if="this.multiselecttitle == true">{{this.title}}</p>
  <div class="hlx-multi-select-input-placeholder" @click="color" v-if="this.multiselectplaceholder == true">{{this.placeholder_value}}</div>
  <div class="hlx-multi-select-input" :id="'select'+this.unique" @click="color" v-show="this.multiselectdropdownvalue == true">
   ...
   <div class="hlx-multi-select-count"  @click="color" v-if="this.all !== true">
    <div class="hlx-multi-select-count-check" v-show="this.count_select_value>0">{{'+'+this.count_select_value}}</div>
    </div>
  <div class="hlx-multi-select-icon" @click="color">
  ...
  <div class="hlx-multi-select-option-container"  v-show="this.isMultiSelect == true" :style="{top:this.top+'px',right:this.right+'px'}">
    <Search class="sea" @searchkey="search" :search_style="'style1'"></Search>
...
<div class="custom-dropdown" @click="Customoptioncontainer" v-if="this.inline_search === true">
<input type="text" class="custom-dropdown-input" :id="'custom-input'+this.unique" v-model="this.selectvalue" :placeholder="this.placeholdervalue" @keypress="senddata">
<i class="icon-angle-down-regular"></i>
<div class="custom-label" v-show="this.animation === true">{{this.placeholder_value}}</div>
...
export default {
  ...
  inheritAttrs:false,
  emits:['customChange','update:value','selectedvalue','update:name','searchvalue','clearablevalue'],
  data () {
    return {
  ...
  mounted () {
    ...
    if (this.multi_select === true) {
      ...
      this.$emit('update:value', this.multiselect_options)
      const value = this.multiselect_options
        .filter(data => {
      ...
      this.all = true
      // this.multiselect_options.forEach((e,index)=>{
      //            e.id = 'singleselect' + this.unique + index
      //         })
    } else if (this.icon == true) {
    ...
    else if(this.inline_search === true)
    {
      this.single_select = false
          if(this.options&& this.options.length>0){
              this.customoption = JSON.parse(JSON.stringify([...this.options]))
            }
            this.placeholdervalue = this.placeholder_value
                if(this.prevalue !== '')
{
    ...
    if(this.prevalue !== '')
{
  ...
  computed: {
    ...
    computeDropdownData () {
      const data = [...this.multiselect_options]
      if (this.search_by && this.search_by != '') {
        ...
        return this.multiselect_options
      }
    },
    dropdowndata()
   {
      ...
      if(this.selectvalue !='' && this.selectvalue)
      {
        ...
        return data.filter((item) => {
          if(typeof(item.name) == 'string'){
            ...
            item.name.toLowerCase().toString().includes(this.selectvalue.toLowerCase())
          }else{
            item.name == this.selectvalue
          }
        })
      }
      else{
  ...
  methods: {
    ...
    color () {
      if (this.isMultiSelect === true) {
        ...
        document.querySelector('.hlx-multi-select-container').classList.remove('add-style')
      } else if (this.isMultiSelect === false) {
        this.isMultiSelect = true
        document.querySelector('.hlx-multi-select-container').classList.add('add-style')
      }
    },
    MultiselectOptioncontainer (event) {
      this.multi_select_value = this.multiselect_options
        .filter(data => {
          if (data.checked == true) {
            this.multiselecttitle = true
            this.multiselectplaceholder = false
    ...
    removemultiselectvalue (removedItem) {
      ...
      this.multiselect_options.filter(value => {
